...
### Phase 6F digest artifact closeout

Phase 6F was separately owner-approved after the Phase 6E task contract. It
created and pushed the approved digest artifact without adding retrieval,
indexing, RAG, external services, release automation, MCP/Hermes, or downstream
integration.
...
| source count | 32 |
| digest algorithm | `sha256` |
| artifact-containing commit | `df51aac8c3a9b001c4d036633897f176b87c304d` |
| source-basis commit recorded in artifact | `37a0e7274ae2cd0a50811c138147a37c1b4c0160` |
| Local Verify evidence | run `27522922010`, job `81344426960`, conclusion `success` |
| release artifact status | `not_release_artifact_without_separate_approval` |
| RAG authorization status | `not_authorized` |

The artifact is metadata/hash-only and does not copy full source text. It is
not a release artifact unless separately approved. It does not authorize RAG,
retrieval, indexing, embeddings, vector storage, external service use,
MCP/Hermes implementation, release automation, or downstream integration.

`08_Study` raw notes, RSID raw evidence, and downstream raw evidence remain
...
The planning tasks through Phase 6E did not create a digest artifact. Phase 6F
later created `artifacts/corpus-digest.json` with separate owner approval.

The existing Phase 6F artifact remains bounded:

- it is metadata/hash-only;
- it records 32 approved source entries;
- it is not a release artifact unless separately approved;
- it does not authorize RAG, retrieval, indexing, embeddings, vector storage,
  external service use, MCP/Hermes, release automation, or downstream
  integration;
- it does not include `08_Study` raw notes, RSID raw evidence, or downstream
  raw evidence.

Any future artifact update or replacement task must separately approve:
...
No additional artifact may be created or regenerated by implication from this
plan or from the Phase 6F artifact.
...
8. plan local RAG over the approved digest basis.

Steps 1-7 are complete for the first approved digest artifact. Step 8 is the
next planning target and must start as Phase 7A local RAG design / read-only
lexical retriever planning, not implementation.
...
Phase 6 planning and approved digest generation alignment succeeds when:
...
- the approved digest artifact exists only because Phase 6F separately approved
  `artifacts/corpus-digest.json`;
- no RAG code, retrieval folder, index folder, corpus folder, embeddings,
  vector database, external service, CI integration, quality-gate integration,
  audit automation, MCP/Hermes implementation, release automation, downstream
  edit, unrelated artifact regeneration, digest regeneration, or eval report
  generation occurs during this alignment.
...
Task:
Plan Phase 7A local RAG design / read-only lexical retriever.

Goal:
Use the completed Phase 6F approved corpus digest artifact as metadata evidence
for a documentation-only local RAG design. Define a read-only lexical retriever
boundary without implementing RAG, retrieval, indexing, embeddings, vector
storage, external services, MCP/Hermes, release automation, or downstream
integration.
...
- changed files
- Phase 7A local RAG design summary
- read-only lexical retriever boundary
- approved digest artifact evidence used
- forbidden corpus confirmation
- verification command results
- safety scan results
- whether a local commit was created
- whether RAG, retrieval, indexing, embeddings, vector storage, external
  service use, digest regeneration, push, tag, release, artifact regeneration,
