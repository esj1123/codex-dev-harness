...
- Phase 5A/5B retain explicit opt-in monolithic and split report evidence plus
  receipt references; reports are not generated by default.
- Commit `d6de357aa0b68bac6ad80b33e3041abb08f57f0b` records
...
- The workflow remains `workflow_dispatch` only with `contents: read`, no
  secrets, no report flags, no artifact generation or upload, no required-check
  semantics, and no release-blocking policy.
- The eval runner and gate remain standalone from `scripts/quality_gate.py`;
  no automatic-trigger or additional CI eval integration is approved.
...
That target is now installed as the manual read-only Local Verify workflow. It
runs pytest, the exact no-report standalone eval command, the quality gate, and
three profile dry-runs. The completed sequence has since advanced through
audit/trace/receipt schema, JSON Evidence Core, eval/report evidence, approved
corpus digest, and standalone local RAG work. The current next recommended task
...
| Eval/report integration | Eval reports cite stable receipt fields and remain explicit opt-in; the exact no-report console command may run in manual read-only Local Verify, while quality-gate, automatic-trigger, required-check, artifact-upload, and release-blocking behavior remain separately approval-gated. |
