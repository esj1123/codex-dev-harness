...
LOCAL_PREDICATE_TYPE = "https://codex-dev-harness.local/provenance/v1"
HOSTED_PREDICATE_TYPE = "https://codex-dev-harness.local/provenance/v2"
LOCAL_BUILDER_ID = "codex-dev-harness-local"
HOSTED_BUILDER_ID = "codex-dev-harness-github-actions-manual-export"
LOCAL_EXECUTION_CONTEXT = "local"
...
def build_statement(
    ...
    execution_context: str = LOCAL_EXECUTION_CONTEXT,
) -> dict[str, Any]:
    ...
    if execution_context == LOCAL_EXECUTION_CONTEXT:
        predicate_type = LOCAL_PREDICATE_TYPE
    ...
    elif execution_context == HOSTED_EXECUTION_CONTEXT:
        predicate_type = HOSTED_PREDICATE_TYPE
        schema_version = HOSTED_SCHEMA_VERSION
        local_only = False
        builder_id = HOSTED_BUILDER_ID
        commands = [
            command.replace(
                " --output artifacts/provenance.intoto.jsonl",
                " --output artifacts/provenance.intoto.jsonl --execution-context github_actions_manual_export",
            )
            if "generate_provenance.py" in command
            else command
            for command in DEFAULT_COMMANDS
        ]
        notes = [
            "Evidence generated by the approval-gated GitHub Actions manual export context.",
            "This record does not claim upload success, attestation, signing, publication, or release.",
            "The provenance file does not include its own digest to avoid self-reference.",
        ]
    else:
    ...
    if execution_context == HOSTED_EXECUTION_CONTEXT:
        predicate["execution_context"] = HOSTED_EXECUTION_CONTEXT
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--execution-context",
        choices=[LOCAL_EXECUTION_CONTEXT, HOSTED_EXECUTION_CONTEXT],
        default=LOCAL_EXECUTION_CONTEXT,
        help="Evidence generation context; local is the compatibility default",
    )
    return parser
