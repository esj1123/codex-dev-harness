...
from scripts.gates.docs_gate import REQUIRED_DOCS
...
def test_current_tree_manifest_passes_with_exact_required_doc_coverage() -> None:
    ...
    assert result["current_state"] == "PRE_LARGE_INTEGRATION_SELF_PILOT"
    assert result["manifest_summary"]["required_doc_count"] == len(REQUIRED_DOCS) == 75
    assert result["manifest_summary"]["classified_required_doc_count"] == 75
